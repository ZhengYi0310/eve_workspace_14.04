...
namespace wam_dmp_controller
{
	...
	bool JointSpaceController::init(hardware_interface::EffortJointInterface *robot, ros::NodeHandle &n)
	{
        ...
        pub_q_err_->msg_.joint_pos.resize(kdl_chain_.getNrOfJoints());
         curr_cart_pos_.reset(new realtime_tools::RealtimePublisher<wam_dmp_controller::PoseRPY>(nh_, "curr_cart_pos", 10));
		return true;		
	}
    ...
    void JointSpaceController::starting(const ros::Time& time)
	{
  		...
  		for(size_t i=0; i<joint_handles_.size(); i++) 
  		{
            ...
            */
            ...
            command_struct_.positions_[i] = joint_msr_states_.q(i);
    ...
    void JointSpaceController::update(const ros::Time& time, const ros::Duration& period)
    {
        ...
        */
        ...
        ee_fk_frame_.M.GetEulerZYX(alpha_, beta_, gamma_);
        p_ws_ee_ = ee_fk_frame_.p;
        command_struct_ = *(command_buffer_.readFromRT());
    ...
    void JointSpaceController::publish_(const ros::Time& time)
    {
        ...
        if ( curr_cart_pos_ && curr_cart_pos_->trylock())
        {
            curr_cart_pos_->msg_.stamp = time;
            curr_cart_pos_->msg_.position.x = p_ws_ee_[0];
            curr_cart_pos_->msg_.position.y = p_ws_ee_[1];
            curr_cart_pos_->msg_.position.z = p_ws_ee_[2];
            curr_cart_pos_->msg_.orientation.roll = alpha_;
            curr_cart_pos_->msg_.orientation.pitch = beta_;
            curr_cart_pos_->msg_.orientation.yaw = gamma_;
        ...
        curr_cart_pos_->unlockAndPublish();
    }
    ...
    {
        /*
        has_velocitycommand_struct_.positions_ = std::vector<double>(q_des.data(), q_des.data() + q_des.rows() * q_des.cols());
