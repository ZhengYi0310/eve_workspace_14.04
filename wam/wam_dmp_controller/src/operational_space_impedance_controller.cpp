...
J_dyn_                  = Eigen::MatrixXd::Zero(6, 6);
    ...
    void OperationalSpaceImpedanceController::update(const ros::Time& time, const ros::Duration& period)
    {
       ...
       // ROS_INFO("EE JACOBIAN COMPUTED!!!!");
        ...
        p_ws_ee_ = R_ws_base_ * (ee_fk_frame.p - p_base_ws_);
        R_ws_ee_.GetEulerZYX(alpha, beta, gamma);  
        //std::cout << "alpha, beta, gamma: " << std::endl <<alpha << ", " << ", " << beta << ", " << gamma  << std::endl;
        ...
        */
        ...
        rot_xyzdot_ws_ << ws_xdot_[3],  ws_xdot_[4],  ws_xdot_[5];
        /*
        std::cout << "joint_position: "<< std::endl << joint_msr_states_.q.data.format(*print_fmt_) << std::endl;
        std::cout << "joint velocity: "<< std::endl << joint_msr_states_.qdot.data.format(*print_fmt_) << std::endl;
        std::cout << "  ws_xdot_: " << ws_xdot_.size() << std::endl << ws_xdot_.format(*print_fmt_) << std::endl;
        std::cout << " rot_xyzdot_ws_: " << rot_xyzdot_ws_.size() << std::endl << rot_xyzdot_ws_.format(*print_fmt_) << std::endl;
        std::cout << " ws_JA_ee: " << ws_JA_ee.size() << std::endl << ws_JA_ee.format(*print_fmt_) << std::endl;
        */
        ...
        //std::cout << "Lambda:" << std::endl << Lambda.format(*print_fmt_) << std::endl;
        command_filter_ = base_J_ee.data.transpose() * Lambda;
        //std::cout << "command_filter_: "<< std::endl << command_filter_.format(*print_fmt_) << std::endl; 
        if(use_simulation_)
        {
       ...
       // std::cout << "Coreolis compensation and jacdot: "<< std::endl <<  tau_.format(*print_fmt_) << std::endl; 
        ...
        for (int i = 0; i < 3; i++)
        {
            if (trans_xyzdotdot_des_ws_[i] <= 1.0e-5)
                trans_xyzdotdot_des_ws_[i] = 0;
            if (trans_xyzdot_des_ws_[i] <= 1.0e-5)
                trans_xyzdot_des_ws_[i] = 0;
            if (trans_xyz_des_ws_[i] <= 1.0e-5)
                trans_xyz_des_ws_[i] = 0;

            if (trans_xyz_ws_[i] <= 1.0e-5)
                trans_xyz_ws_[i] = 0;
            if (trans_xyzdot_ws_[i] <= 1.0e-5)
                trans_xyzdot_ws_[i] = 0;
        }

        for (int i = 0; i < 3; i++)
        { 
            if (rot_xyzdotdot_des_ws_[i] <= 1.0e-5)
                rot_xyzdotdot_des_ws_[i] = 0;
            if (rot_xyzdot_des_ws_[i] <= 1.0e-5)
                rot_xyzdot_des_ws_[i] = 0;
            if (rot_xyz_des_ws_[i] <= 1.0e-5)
                rot_xyz_des_ws_[i] = 0;

            if (rot_xyz_ws_[i] <= 1.0e-5)
                rot_xyz_ws_[i] = 0;
            if (rot_xyzdot_ws_[i] <= 1.0e-5)
                rot_xyzdot_ws_[i] = 0;
        }
        */
        ...
        tau_trans_ = trans_xyzdotdot_des_ws_ + Kp_.block(0, 0, 3, 3) * trans_xyz_error_ + Kv_.block(0, 0, 3, 3) * (trans_xyzdot_des_ws_ - trans_xyzdot_ws_);
       
        /*
        for (int i = 0; i < 3; i++)
        {
            rot_xyz_error_(i) = angles::normalize_angle(rot_xyz_error_(i));
            if (rot_xyz_error_[i] <= 1.0e-5)
                rot_xyz_error_[i] = 0;
            if (trans_xyz_error_[i] <= 1.0e-5)
                trans_xyz_error_[i] = 0; 
        }
        */
        /*
        std::cout << "trans_xyz_error_: " << std::endl << trans_xyz_error_.format(*print_fmt_) << std::endl;
        std::cout << "trans_xyz_dot_error_: " << std::endl << (trans_xyzdot_des_ws_ - trans_xyzdot_ws_).format(*print_fmt_) << std::endl;
        std::cout << "trans_xyzdotdot_des_ws_: " << std::endl<<  trans_xyzdotdot_des_ws_.format(*print_fmt_) << std::endl;
        std::cout << "trans_xyzdot_ws_: " << std::endl<<  trans_xyzdot_ws_.format(*print_fmt_) << std::endl;
        std::cout << "trans_xyz_ws_: " << std::endl<<  trans_xyz_ws_.format(*print_fmt_) << std::endl;
        */
       
        tau_rot_ = rot_xyzdotdot_des_ws_ + Kp_.block(3, 3, 3, 3) * rot_xyz_error_ + Kv_.block(3, 3, 3, 3) * (rot_xyzdot_des_ws_ - rot_xyzdot_ws_);
        /*
        std::cout << "rot_xyz_error_: " << std::endl << rot_xyz_error_.format(*print_fmt_) << std::endl;
        std::cout << "rot_xyz_dot_error: " << std::endl << (rot_xyzdot_des_ws_ - rot_xyzdot_ws_).format(*print_fmt_) << std::endl;
        
        std::cout << "rot_xyzdotdot_des_ws_: " << std::endl << rot_xyzdotdot_des_ws_.format(*print_fmt_) << std::endl;
        std::cout << "rot_xyzdot_ws_: " << std::endl <<rot_xyzdot_ws_.format(*print_fmt_) << std::endl;
        std::cout << "rot_xyz_ws_: " << std::endl <<rot_xyz_ws_.format(*print_fmt_) << std::endl;

        std::cout << "Kp: " << std::endl << Kp_.format(*print_fmt_) << std::endl;
        std::cout << "Kv: " << std::endl << Kv_.format(*print_fmt_) << std::endl;
        */
        ...
        //std::cout << "F_unit_: " << std::endl << F_unit_.format(*print_fmt_) << std::endl; 
        /* 
        for (int i = 0; i < 6; i++)
        {
        ...
        */
        ...
        //std::cout << "filtered_f_unit: "<< std::endl << (command_filter_ * F_unit_).format(*print_fmt_) << std::endl; 
        /*
        for (int i = 0; i < 7; i++)
        {
        ...
        */
    	...
    	// set joint efforts
       // std::cout << "final torque!!!!" <<tau_.format(*print_fmt_) << std::endl;
