/*************************************************************************
	> File Name: sim_hw.cpp
	> Author: Yi Zheng 
	> Mail: hczhengcq@gmail.com
	> Created Time: Fri 16 Feb 2018 10:02:48 AM PST
 ************************************************************************/

#include "sim_hw/base_robot_hw.h"
namespace sim_hw
{
    int BaseRobotHW:: n_joints_ = 7;
    void BaseRobotHW::create(std::string name, std::string urdf_string)
    {
    ...
    void BaseRobotHW::registerInterfaces(const urdf::Model *const urdf_model, 
                              std::vector<transmission_interface::TransmissionInfo> transmissions)
    {
        // Check that this transmission has one joint
        if( transmissions.empty() )
        {
            std::cout << "base_robot_hw: " << "There are no transmission in this robot, all are non-driven joints? " 
            << std::endl;
            return;
        }
        ...
        for(int j=0; j < n_joints_; j++)
        {
            ...
            {
                std::cout << "base_robot_hw: " << "Transmission " << transmissions[j].name_
            ...
            {
                std::cout << "base_robot_hw: " << "Joint " << transmissions[j].joints_[0].name_ <<
                " of transmission " << transmissions[j].name_ << " does not specify any hardware interface. " << "You need to, otherwise the joint can't be controlled." << std::endl;
                continue;
            }
        ...
        for(int j=0; j < 12; ++j)
        {
            cart_interface_.registerHandle(barrett_model::CartesianStateHandle(
            cart_12_names_[j], &cart_pos_[j], &cart_stiff_[j], &cart_damp_[j]));
            barrett_model::CartesianHandle cart_pos_handle;
            cart_pos_handle = barrett_model::CartesianHandle(cart_interface_.getHandle(cart_12_names_[j]), 
                                                                  &cart_pos_command_[j]);
        ...
        for(int j=0; j < 6; ++j)
        {
            ...
            barrett_model::CartesianHandle cart_stiff_handle;
            cart_stiff_handle = barrett_model::CartesianHandle(cart_interface_.getHandle(cart_6_names_[j] + std::string("_stiffness")),
      ...
      barrett_model::CartesianHandle cart_damp_handle;
            cart_damp_handle = barrett_model::CartesianHandle(cart_interface_.getHandle(cart_6_names_[j] + std::string("_damping")),
            ...
            barrett_model::CartesianHandle cart_wrench_handle;
            cart_wrench_handle = barrett_model::CartesianHandle(cart_interface_.getHandle(cart_6_names_[j] + std::string("_wrench")),
    ...
    void BaseRobotHW::enforceLimits(ros::Duration period)
    {
    ...
    bool BaseRobotHW::parseTransmissionsFromURDF(const std::string& urdf_string)
    {
        std::vector<transmission_interface::TransmissionInfo> transmissions;
    ...
    // Init KDL stuff
    bool BaseRobotHW::initKDLdescription(const urdf::Model *const urdf_model)
    {
        ...
        std::cout << "BaseRobot kinematic successfully parsed with " 
        ...
        ROS_INFO("Number of segments: %d", robot_chain_.getNrOfSegments());
        ROS_INFO("Number of joints in chain: %d", robot_chain_.getNrOfJoints());

        f_dyn_solver_.reset(new KDL::ChainDynParam(robot_chain_, gravity_));

        joint_position_kdl_ = KDL::JntArray(robot_chain_.getNrOfJoints());
        gravity_effort_ = KDL::JntArray(robot_chain_.getNrOfJoints());

        return true;
    }

#if ROS_VERSION_MINIMUM(1,12,6)
    bool BaseRobotHW::prepareSwitch(const std::list<hardware_interface::ControllerInfo> &start_list, const std::list<hardware_interface::ControllerInfo> &stop_list) const
    {
...
#else
    bool BaseRobotHW::canSwitch(const std::list<hardware_interface::ControllerInfo> &start_list, const std::list<hardware_interface::ControllerInfo> &stop_list) const
    {
        int counter_position = 0, counter_effort = 0, counter_cartesian = 0;
        ...
        for ( std::list<hardware_interface::ControllerInfo>::const_iterator it = start_list.begin(); it != start_list.end(); ++it )
        {
            ...
            if( it->hardware_interface.compare( std::string("hardware_interface::VelocityJointInterface") ) == 0 )
            {
                std::cout << "The given controllers to start work on a velocity joint interface, and this robot does not have such an interface." << "The switch can't be done" << std::endl;
                return false;
            }
...
#endif
    void BaseRobotHW::doSwitch(const std::list<hardware_interface::ControllerInfo> &start_list, const std::list<hardware_interface::ControllerInfo> &stop_list)
    {
...
#if ROS_VERSION_MINIMUM(1,12,6)
    BaseRobotHW::ControlStrategy BaseRobotHW::getNewControlStrategy(const std::list< hardware_interface::ControllerInfo >& start_list, const std::list< hardware_interface::ControllerInfo >& stop_list, BaseRobotHW::ControlStrategy default_control_strategy)
    {
