/*************************************************************************
	> File Name: barrett_hw.h
	> Author: Yi Zheng 
	> Mail: hczhengcq@gmail.com
	> Created Time: Sun 04 Mar 2018 01:29:29 PM PST
 ************************************************************************/

#ifndef _BARRETT_HW_H
#define _BARRETT_HW_H
...
{
    class BarrettWAMHW : public BaseRobotHW
    {
        public:
            EIGEN_MAKE_ALIGNED_OPERATOR_NEW
            BarrettWAMHW(ros::NodeHandle nh);
            bool configure();
            bool start();
        ...
        protected:
            ...
            template<size_t DOF>
            boost::shared_ptr<BarrettWAMHW::WamDevice<DOF> > configure_wam(ros::NodeHandle product_nh, boost::shared_ptr<barrett::ProductManager> barrett_manager, const libconfig::Setting &wam_config);

            template<size_t DOF>
            bool read_wam(const ros::Time time, const ros::Duration period, boost::shared_ptr<BarrettWAMHW::WamDevice<DOF> > device);

            template<size_t DOF>
            void write_wam(const ros::Time time, const ros::Duration period, boost::shared_ptr<BarrettWAMHW::WamDevice<DOF> > device);
