...
namespace task_recorder 
{
    ...
    class TaskRecorder : public TaskRecorderBase
    {
        public:
            ...
            {
                task_recorder::TaskRecorderSpecification task_recorder_specification;
             ...
             */
            bool initialize(const task_recorder::TaskRecorderSpecification& task_recorder_specification);
            ...
            bool readParams(ros::NodeHandle& node_handle, const std::string& class_name_prefix)
            {
                ...
                if (!class_name_prefix.empty())
                {
                    ...
                    {
                        ROS_ERROR("The node doesn't contain the variable_name_prefix in the parameter server.");
                    }
                }
             ...
             * @return True on success, otherwise False
             */
            bool startStreaming(task_recorder::StartStreaming::Request& request, task_recorder::StartStreaming::Response& response);
             ...
             * @return True on success, otherwise False
             */
            bool stopStreaming(task_recorder::StopStreaming::Request& request, task_recorder::StopStreaming::Response& response);
             ...
             * @return True on success, otherwise False
             */
            bool stopRecording(task_recorder::StopRecording::Request& request, task_recorder::StopRecording::Response& response);
             ...
             * @return True on success, otherwise False
             */
            bool getSampleData(const ros::Time& time, task_recorder::DataSample& data_sample);
        ...
        protected:
             ...
             * @param num_samples
             * @param message_names
             * @param filtered_and_cropped_messages
             * @return True on success, otherwise False 
             */
            bool filterAndCrop(const ros::Time& start_time,
                               ...
                               std::vector<task_recorder::DataSample>& filtered_and_cropped_messages);
             ...
             * @return True on success, otherwise False
             */
            virtual bool transformMsg(const MessageType& message, task_recorder::DataSample& data_sample_msg) = 0;
        ...
        private:
            ...
            /*!
             */
            // task_recorder_utilities::Accumulator accumulator_;
            boost::shared_ptr<task_recorder_utilities::MessageRingBuffer> message_buffer_;
            // boost::shared_ptr<task_recorder_utilities::MessageBuffer> message_buffer_;
            task_recorder::DataSample data_sample_;
             ...
             * @param description
             */
            void startRecording(const task_recorder::Description& description);
             ...
             * @return True on success, otherwise False
             */
            bool resample(std::vector<task_recorder::DataSample>& messages,
                          const ros::Time& start_time,
                          const ros::Time& end_time,
                          const int num_samples,
                          const std::vector<std::string>& message_names,
                          std::vector<task_recorder::DataSample>& resampled_messages);

            /*!
             * @param data_sample
             * @return True on success, otherwise False
             */
            bool filter(task_recorder::DataSample& data_sample);
    ...
    template<class MessageType>
    bool TaskRecorder<MessageType>::initialize(const task_recorder::TaskRecorderSpecification& task_recorder_specification)
    {
        ...
        if(!task_recorder_utilities::getTopicName(full_topic_name))
        {
    ...
    void TaskRecorder<MessageType>::recordMessagesCallback(const MessageTypeConstPtr message)
    {
        ...
        {
            ROS_ERROR("Can't transform message to task_recorder::DataSample class!");
            return;
        }
    ...
    template<class MessageType>
    void TaskRecorder<MessageType>::startRecording(const task_recorder::Description& description)
    {
        ROS_INFO("Start recording topic named >%s< with description >%s< to id >%i<.", recorder_io_.topic_name_.c_str(), task_recorder_utilities::getDescription(description).c_str(), task_recorder_utilities::getId(description));
        ...
        waitForMessages();
        ROS_INFO("Recording topic named >%s< with description >%s< to id >%i<.", recorder_io_.topic_name_.c_str(), task_recorder_utilities::getDescription(description).c_str(), task_recorder_utilities::getId(description));
    }
    ...
    template<class MessageType>
    bool TaskRecorder<MessageType>::startStreaming(task_recorder::StartStreaming::Request& request,
                                                   task_recorder::StartStreaming::Response& response)
    {
        setStreaming(true);
        response.info = std::string("Started streaming >" + recorder_io_.topic_name_ + "<. ");
        response.return_code = task_recorder::StartStreaming::Response::SERVICE_CALL_SUCCESSFUL;
        return true;
    }

    template<class MessageType>
    bool TaskRecorder<MessageType>::stopStreaming(task_recorder::StopStreaming::Request& request,
    ...
    //   boost::thread(boost::bind(&task_recorder_utilities::TaskRecorderIO<task_recorder::DataSample>::writeStatistics, recorder_io_, vector_of_accumulated_trial_statistics));
    // }

        ROS_DEBUG("Stopped recording topic >%s< and returning >%i< messages.", recorder_io_.topic_name_.c_str(), (int)response.filtered_and_cropped_messages.size());
    ...
    template<class MessageType>
    bool TaskRecorder<MessageType>::interruptRecording(task_recorder::InterruptRecording::Request& request,
                                                       task_recorder::InterruptRecording::Response& response)
    {
        setLogging(false);
        ROS_DEBUG("Interrupted recording topic >%s<.", recorder_io_.topic_name_.c_str());
        response.info = std::string("Stopped recording >" + recorder_io_.topic_name_ + "<. ");
        response.return_code = task_recorder::InterruptRecording::Response::SERVICE_CALL_SUCCESSFUL;
        return true;
    }
                                                  ...
                                                  std::vector<task_recorder::DataSample>& filtered_and_cropped_messages)
    {
        ...
        return filterAndCrop(start_time, end_time, num_samples, no_message_names, filtered_and_cropped_messages);
    }
                                                  ...
                                                  std::vector<task_recorder::DataSample>& filtered_and_cropped_messages)
    {
        ...
        if(num_samples == 1) // only 1 sample requested
        {
            ...
            filtered_and_cropped_messages.push_back(data_sample);
    ...
    template<class MessageType>
    bool TaskRecorder<MessageType>::resample(std::vector<task_recorder::DataSample>& messages,
                                             const ros::Time& start_time,
                                             const ros::Time& end_time,
                                             const int num_samples,
                                             const std::vector<std::string>& message_names,
                                             std::vector<task_recorder::DataSample>& resampled_messages)
    {
        ...
        ROS_VERIFY(task_recorder_utilities::getIndices(messages[0].names, selected_names, indices));
    ...
    template<class MessageType>
    bool TaskRecorder<MessageType>::filter(task_recorder::DataSample& data_sample)
    {
    ...
    template<class MessageType>
    bool TaskRecorder<MessageType>::getSampleData(const ros::Time& time, task_recorder::DataSample& data_sample)
    {
        if(isStreaming())
        {
